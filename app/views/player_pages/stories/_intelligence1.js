var story = {
  //order, dialog, character, side
  "dialogs": [[1, "If Jerry colleted 68 melons and John collected 76, hoy many melons should be given to the 36 members of our tribe?", "nicubunu_Game_baddie_Squared.png", "left"],
  [0, "Ermm I dont know, that sounds like a looot of melons, lets just give 3 melons to everyone and the rest we offer to the gods", "nicubunu_Game_baddie_Basic_guy.png", "right"],
  [1, "Aaactuallyyy... its not 'a lot', its exactly 144 melons, and the gods don't need our melons, we can just give exactly 4 to every one", "nicubunu_Game_baddie_Squared.png", "left"],
  [0, "Wow, are you sure?, That sounds great!!, But I'm actually a little afraid that the gods will get mad if we start forgetting them", "nicubunu_Game_baddie_Basic_guy.png", "right"],
  [1, "Who needs them if we have math, as long as we are able to understand this world", "nicubunu_Game_baddie_Squared.png", "left"],
  [0, "But can that math you talk about really help explain everything in this world?, like why it rains or how good would the crops be", "nicubunu_Game_baddie_Basic_guy.png", "right"],
  [1, "Well, not right now, but with enough studies YES!, YES IT CAN!", "nicubunu_Game_baddie_Squared.png", "left"],
  [0, "Hmm,I guess that as long as we live in this world you might be right", "nicubunu_Game_baddie_Basic_guy.png", "right"]],
  //background, frame, tutorial? (small frame)
  "scenes": [["background_caveBmath.jpg", 0, false]],
  //strength, intelligence, curiosity, organization, construction
  "levels": [200,0,0,0,0]
}
